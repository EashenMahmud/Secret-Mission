import React from 'react';
import { Formik, Form } from 'formik';
import * as Yup from 'yup';
import Modal from '../../components/ui/Modal';
import Input from '../../components/ui/Input';
import Switch from '../../components/ui/Switch';
import Button from '../../components/ui/Button';
import { usePostApiMutation } from '../../store/api/commonApi';
import { Building2 } from 'lucide-react';
import { toast } from 'react-toastify';

const departmentSchema = Yup.object().shape({
    name: Yup.string().required('Department name is required'),
    code: Yup.string().required('Department code is required'),
});

const DepartmentForm = ({ isOpen, onClose, department, onSuccess }) => {
    const isEditing = !!department;

    const initialValues = {
        name: department?.name || '',
        code: department?.code || '',
        is_active: department?.is_active ?? true,
    };

    const [saveDepartment, { isLoading }] = usePostApiMutation();

    const handleSubmit = async (values, { setSubmitting }) => {
        try {
            // Convert is_active to 1 or 0 for backend if needed
            const body = {
                ...values,
                is_active: values.is_active ? 1 : 0
            };

            if (isEditing) {
                await saveDepartment({
                    end_point: `/update-department/${department.id}`,
                    body: body
                }).unwrap();
                toast.success('Department updated successfully');
            } else {
                await saveDepartment({
                    end_point: '/add-department',
                    body: body
                }).unwrap();
                toast.success('Department created successfully');
            }
            onSuccess();
            onClose();
        } catch (err) {
            toast.error(err?.data?.message || 'Failed to save department');
        } finally {
            setSubmitting(false);
        }
    };

    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={isEditing ? 'Edit Department' : 'Add New Department'}
            size="md"
        >
            <Formik
                initialValues={initialValues}
                validationSchema={departmentSchema}
                onSubmit={handleSubmit}
                enableReinitialize
            >
                {({ isSubmitting }) => (
                    <Form className="space-y-6 pt-2">
                        <div className="bg-[var(--bg-app)] p-6 rounded-2xl border border-[var(--border-main)] flex flex-col items-center mb-4">
                            <div className="w-16 h-16 rounded-2xl bg-primary-500/10 flex items-center justify-center mb-4 border border-primary-500/20">
                                <Building2 className="w-8 h-8 text-primary-500" />
                            </div>
                            <h3 className="text-lg font-bold text-[var(--text-main)]">Department Details</h3>
                            <p className="text-xs text-[var(--text-muted)] text-center mt-1">
                                Define the department's name and identification code.
                            </p>
                        </div>

                        <div className="space-y-4">
                            <Input
                                label="Department Name"
                                name="name"
                                placeholder="e.g. Engineering, Marketing"
                                required
                            />

                            <Input
                                label="Department Code"
                                name="code"
                                placeholder="e.g. BBDPT01"
                                required
                            />
                        </div>

                        <div className="flex items-center justify-between p-4 bg-[var(--bg-app)] rounded-xl border border-[var(--border-main)]">
                            <div className="flex flex-col">
                                <span className="text-sm font-semibold text-[var(--text-main)]">Active Status</span>
                                <span className="text-xs text-[var(--text-muted)]">Enable or disable this department</span>
                            </div>
                            <Switch name="is_active" />
                        </div>

                        <div className="flex justify-end gap-3 pt-4 border-t border-[var(--border-main)] -mx-6 px-6 -mb-6 pb-6 bg-[var(--bg-app)] rounded-b-xl">
                            <Button variant="ghost" onClick={onClose} type="button">Discard</Button>
                            <Button type="submit" isLoading={isSubmitting || isLoading} className="px-8">
                                {isEditing ? 'Update Dept' : 'Create Dept'}
                            </Button>
                        </div>
                    </Form>
                )}
            </Formik>
        </Modal>
    );
};

export default DepartmentForm;
