        ...
        <div className="relative">
            ...
            <div className="card p-0 bg-transparent border-none">
                <div className="relative py-24">
                    ...
                    <div className="absolute top-1/2 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[var(--border-main)] to-transparent -translate-y-1/2" />

                    <div className="flex items-center justify-center gap-20  pb-12 hide-scrollbar px-10 relative z-10">
                            ...
                            return (
                                <div key={module.id} className="relative flex flex-col items-center">
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            "group relative w-32 h-32 rounded-full flex flex-col items-center justify-center transition-all duration-500",
                                    ...
                                    >
                                        ...
                                        <div className={cn("relative z-30 transition-all duration-500 group-hover:scale-110 group-hover:-translate-y-1 group-hover:text-white", config.text)}>
                                            <ModuleIcon className="w-10 h-10 stroke-[1.5]" />
                                        </div>

                                        {/* Progress Text - Small at bottom */}
                                        <div className="mt-1 relative z-30">
                                            <span className={cn("text-[10px] font-black tracking-widest uppercase opacity-80 group-hover:opacity-100 group-hover:text-white transition-colors duration-500", config.text)}>
                                                {module.progress ?? 0}%
                                            </span>
                                        </div>
                                        ...
                                        <div className="absolute inset-0 z-20 bg-primary-600 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out flex items-end justify-center pb-2">
                                            <ChevronRight className="w-6 h-6 text-white/50 animate-bounce" />
                                        </div>
                                    </button>

                                    {/* Labels Section */}
                                    <div className="absolute -bottom-16 w-48 text-center flex flex-col items-center gap-1">
                                        ...
                                        <h3 className="font-black text-[var(--text-main)] text-sm leading-tight tracking-tight uppercase max-w-[140px] line-clamp-2">
                                            {module.name}
                                        </h3>
                                    </div>

                                    {/* Connecting Stem inside the row */}
                                    <div className="absolute top-1/2 -translate-y-full mb-16 h-8 w-px bg-gradient-to-t from-[var(--border-main)] to-transparent opacity-20" />
                                </div>
                            );
                        })}
                    </div>
            ...
            <div className="mt-12 flex justify-center gap-10">
                {Object.entries(STATUS_CONFIG).map(([key, config]) => (
                    <div key={key} className="flex items-center gap-2.5">
                        <div className={cn("w-3 h-3 rounded-full border-2", config.border, config.bg)} />
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)] opacity-70">
                            {config.label}
                        </span>
                    </div>
                ))}
